function vf_module_table(input) {
	var tables = document.getElementsByTagName("TABLE");
	for (var i=tables.length-1; i>=0;i-=1)
  		if (tables[i]) tables[i].parentNode.removeChild(tables[i]);

  	var table = document.createElement("TABLE");
    table.setAttribute("id", "vf_module_table");
    document.getElementById("newCust").appendChild(table);

	var Th = document.createElement("TH");
    Th.setAttribute("id", "myTh");
    Th.colSpan = 2;
    document.getElementById("vf_module_table").appendChild(Th);
    ...
    for(var key in input){
    	if(key != "vf-module-name"){
	    	...
	    	y.setAttribute("id", count);
	   		document.getElementById("vf_module_table").appendChild(y);
